...
class General extends \common\components\BaseActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['web_name_tw', 'web_name_cn', 'web_name_en'], 'string', 'max' => 255],
            // [['copyright_tw', 'copyright_cn', 'copyright_en', 'call_for_abstract_tw', 'call_for_abstract_cn', 'call_for_abstract_en', 'registration_tw', 'registration_cn', 'registration_en'], 'string'],
            [['copyright_tw', 'copyright_cn', 'copyright_en', 'registration_tw', 'registration_cn', 'registration_en'], 'string'],
            // [['copyright_notice_tw', 'copyright_notice_cn', 'copyright_notice_en', 'disclaimer_tw', 'disclaimer_cn', 'disclaimer_en', 'privacy_statement_tw', 'privacy_statement_cn', 'privacy_statement_en'], 'string'],
            [['description', 'keywords'], 'string'],
            [['banner_image_file_name'], 'string'],
            [['created_at', 'updated_at'], 'safe'],
            // [['delivery_fas_code_id', 'site_counter', 'updated_UID'], 'integer'],
            [['image_file'], 'file', 'skipOnEmpty' => true, 'extensions' => 'png, jpg, JPG, jpeg, JPEG, PNG, gif'],
            // [['crop_width', 'crop_height'], 'integer'],
            // ['crop_info', 'safe'],
            // ['image_file', 'checkClickCrop'],
            // [['delivery_fas_code_id'], 'required'],
            // [['delivery_fas_code_id'], 'exist', 'skipOnError' => true, 'targetClass' => FasCode::className(), 'targetAttribute' => ['delivery_fas_code_id' => 'id']],
        ];
    }
    ...
    public function attributeLabels()
    {
        return [
            ...
            // 'call_for_abstract_tw' => Yii::t('app', 'Content').' '.Yii::t('app', '(Traditional Chinese Version)'),
            // 'call_for_abstract_cn' => Yii::t('app', 'Content').' '.Yii::t('app', '(Simplified Chinese Version)'),
            // 'call_for_abstract_en' => Yii::t('app', 'Content').' '.Yii::t('app', '(English Version)'),
            'registration_tw' => Yii::t('app', 'Content').' '.Yii::t('app', '(Traditional Chinese Version)'),
            'registration_cn' => Yii::t('app', 'Content').' '.Yii::t('app', '(Simplified Chinese Version)'),
            'registration_en' => Yii::t('app', 'Content').' '.Yii::t('app', '(English Version)'),
            ...
            // 'privacy_statement_en' => Yii::t('app', 'Privacy Statement').' '.Yii::t('app', '(English Version)'),
            // 'privacy_statement_cn' => Yii::t('app', 'Privacy Statement').' '.Yii::t('app', '(Simplified Chinese Version)'),
            // 'privacy_statement_tw' => Yii::t('app', 'Privacy Statement').' '.Yii::t('app', '(Traditional Chinese Version)'),
            ...
            // 'delivery_fas_code_id' => Yii::t('fas', 'Delivery FAS Code'),
            ...
            'image_file' => '預設 Logo',
            'banner_image_file_name' => '預設 Logo',
        ];
    }
...
//                 }
            $this->save();
            if ($this->image_file!=NULL) {
                $this->banner_image_file_name = '0.'.$this->image_file->getExtension();
                $this->saveImage($this->banner_image_file_name);
                // $this->save();
            }
            return true;
        } else {
    ...
    //     if (Yii::$app->language == 'en' && !empty($this->call_for_abstract_en)) {
    //         return $this->call_for_abstract_en;
    //     } else if (Yii::$app->language == 'zh-CN' && !empty($this->call_for_abstract_cn)) {
    //         return $this->call_for_abstract_cn;
    //     }
    //     return $this->call_for_abstract_tw;
    // }

    public function getRegistration() {
        if (Yii::$app->language == 'en' && !empty($this->registration_en)) {
            return $this->registration_en;
        } else if (Yii::$app->language == 'zh-CN' && !empty($this->registration_cn)) {
            return $this->registration_cn;
        }
        return $this->registration_tw;
    }
    ...
    // public function getDeliveryFasCode()
    // {
    //     return $this->hasOne(FasCode::className(), ['id' => 'delivery_fas_code_id']);
    // }
}
